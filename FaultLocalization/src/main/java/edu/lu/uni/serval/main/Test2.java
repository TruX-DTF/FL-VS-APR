...
//		builder.setLength(0);
	...
	public void locateSuspiciousCode(String path, String buggyProject, String outputPath) throws IOException {
		...
		File dataPathFile = new File("../Data/suspiciousPositions/" + buggyProject + "/");
		...
		for (String metricStr : Configuration.METRICS_0_1_1) {
			File dataFile = new File("../Data/suspiciousPositions/" + buggyProject + "/" + metricStr + ".txt");
			if (!dataFile.exists()) {
				builder.append("-,");
				continue;
			}
			List<String> suspCodeList = Arrays.asList(FileHelper.readFile(dataFile).split("\n"));
			...
			for (int index = 0, size = suspCodeList.size(); index < size; index ++) {
				...
				int lineNumber = Integer.valueOf(candidate.substring(atIndex + 1));
