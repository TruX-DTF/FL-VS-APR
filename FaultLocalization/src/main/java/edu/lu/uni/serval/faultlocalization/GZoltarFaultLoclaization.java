	...
	public void sortSuspiciousCode() {
		...
		for (Statement gzoltarStatement : suspiciousStatements) {
            ...
            while (nextTest != -1) {
            	...
            	String testCase = testResult.getName();
...
//		Collections.sort(candidates, (c1, c2) -> Double.compare(c2.getSuspiciousValue(), c1.getSuspiciousValue()));
	...
	private void parseTestResults() {
        List<String> passingTestCases = new ArrayList<String>();
...
//			}
			if (tr.wasSuccessful()) {
				if (!passingTestCases.contains(testName)) passingTestCases.add(testName);
			} else {
...
//                }
		...
		totalPassedTestCases = passingTestCases.size();
		totalFailedTestCases = failingTestCases.size();
